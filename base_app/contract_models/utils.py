...
def start_client(data: dict, contract, filial):
...
def save_to_xml(data: dict, type_order, contract, filial):
    for k1, v1 in data.items():
        ...
        for key, value in data[k1].items():
            ...
            for k, v in value.items():
                ...
                row.attrib = {'name': k}
                row.text = str(v)
        save_file_name = (__create_name_file_save_xml
                          (filename=str(k1), const_name=str(type_order), contract=contract, filial=filial)
                          )
        __save_xml(save_file_name, new)
...
def __create_name_file_save_xml(filename, const_name, contract, filial):
    dt = __create_date_file_name()
    wb_path = os.path.join(filial.path_saved_order, contract.name, 'Заявки')
