...
def start(file_name, contract, filial):
    ...
    try:
        ...
        if 'НЕО-ТРЕЙД' in dic_const['comment']:
            __create_porder_data(_df=df, _contract=contract, _filial=filial)
        else:
            __create_order_data(_df=df, _contract=contract, _filial=filial)
...
def __create_order_data(_df, _contract, _filial):
    ...
    save_to_xml(dic_order, 'CustPicking', contract=_contract, filial=_filial)
    dic_log_return['Расход'] += len(dic_order)


def __create_porder_data(_df, _contract, _filial):
    ...
    save_to_xml(dic_porder, 'VendReceipt', _filial)
